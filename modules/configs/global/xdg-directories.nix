{ lib, ... }: let
  ...
  xdgDirsConfig = homeDirectory: builtins.mapAttrs (name: value: lib.mkOption {
    type = with lib.types; nullOr (if (lib.strings.hasSuffix "_DIRS" name) then envVar else str);
    default = value;
  }) (xdgDirectories homeDirectory);

in{

  flake.wrappers.config-xdg-directories = { config, wlib, ... }: {
    imports = [ wlib.modules.default ];
    options.xdgDirectories = (xdgDirsConfig "$HOME");
    config.env = builtins.mapAttrs (name: value: lib.mkDefault { data = value; esc-fn = x: "\"${x}\""; } ) config.xdgDirectories;
  };

  perSystem = {
    wrappers.packages = {
      config-xdg-directories = true;
    };
  };

  flake.homeModules.config-xdg-directories = { config, ... }: {
    options.xdgDirectories = xdgDirsConfig config.home.homeDirectory;
  };

  flake.modules.darwin.config-xdg-directories = { config, ... }: {
